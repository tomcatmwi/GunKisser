$(document).ready(function() {
  ...
  $('#newBtn').click(function(e) {
    e.preventDefault();
    document.location.href = api_url+'/calibers_edit';
  });
...
//  display records
          ...
          for (var t in data.results) {
            ...
            $(temp).find('td:nth-child(2)').find('a:first').attr('href', '/calibers_edit/'+data.results[t]._id);
            $(temp).find('td:nth-child(3)').find('a:first').attr('href', '/calibers_delete/'+data.results[t]._id);
